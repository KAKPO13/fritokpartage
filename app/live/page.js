...
export default function LivePage({ searchParams }) {
  ...
  // Agora init (audience)
  useEffect(() => {
    if (typeof window === "undefined" || !channel || !token) return;
    ...
    async function initAgora() {
      ...
      client.on("user-joined", updateCountWithAnimation);
      client.on("user-left", updateCountWithAnimation);
  ...
  return (
    <main className="live-container">
      ...
      {/* LIVE badge avec compteur */}
      <div className="live-badge" aria-label="Live en cours">
        <span className="dot" /> LIVE
        <span className={`audience-count ${animateCount ? "animate" : ""}`}>
