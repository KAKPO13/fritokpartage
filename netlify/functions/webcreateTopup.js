...
// ➜ Enregistre une entrée TransfetMoney type "topup" en "pending"
//   (mise à jour en "completed" par le webhook Flutterwave)
//
// ⚠️ Converti en ESM (export const handler au lieu de exports.handler) :
// le package.json a "type": "module", donc `exports.handler = ...` ne
// produit pas un export réellement reconnu par le runtime Lambda —
// c'est exactement ce qui causait le crash observé :
//   Runtime.HandlerNotFound: webcreateTopup.handler is undefined or not exported
// ─────────────────────────────────────────────────────────────────────────────
...
export const handler = async (event) => {
  ...
  try {
    ...
    // 6. TransfetMoney — statut "pending" (mis à jour après confirmation FLW)
    ...
    // 7. topupAttempts pour traçabilité + lien avec TransfetMoney
